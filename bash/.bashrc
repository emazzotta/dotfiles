...
alias bup='echo "Updating Brew";git -C "$(brew --repo)" fetch --tags;brew update;brew upgrade;brew upgrade --cask --greedy;brew cleanup;brew cu -afy --cleanup;brew cleanup;rmbrewpkg'
...
alias reinstall_py='av && pip install ptpython && pip install boto3 && pip install poetry'
...
alias rmbrewpkg='find "/opt/homebrew/Caskroom" -type f -name "*.pkg" -delete && rm -rf "$HOME/Library/Caches/Homebrew/downloads" && mkdir -p "$HOME/Library/Caches/Homebrew/downloads"'
...
alias superocd='ocd && update && rmraycastclipboard && gupallin "$HOME" && zgen update <<< "n" &> /dev/null && gck "$HOME" && envify && tmbackup && bookmarks_backup && notion_backup'
alias t='tig'
