...
alias mpc='envify && 7z x "$ZIPS_DIR/MPC.7z" -o"$DESKDIR" -p"$PASSWORD_ZIPS"'
...
alias portal_reset='yarn && killportlisteners && docker-compose down && docker-compose up -d database && yarn clean && yarn generate && yarn build && yarn dev'
...
alias superocd='sshkeyadd && ocd && update && rmraycastclipboard && rm_act_cache && rmmac && gupallin "$HOME" && zgen update <<< "n" &> /dev/null && gck "$HOME" && create_backups'
alias t='lazygit'
alias tags_dj_fix='envify && av && audiotags_manager --autofix'
