...
alias rrr='rm -rf'
alias leoftp='envify && lftp -e "set ssl:verify-certificate no; open ftp://${LEO_FTP_USER}:${LEO_FTP_PASS}@${LEO_FTP_URL}"'
...
alias superocd='sshkeyadd && ocd && update && rmraycastclipboard && kill_unwanted_processes && rm_launch_items && rmmac && upallin "$HOME" && zgen update <<< "n" &> /dev/null && ck "$HOME" && create_backups'
alias t='lazygit'
