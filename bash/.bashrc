...
export VENV_PYTHON_3="${VENV_PATH}/generic-3.8"
...
alias bup='echo "Updating Brew";git -C "$(brew --repo)" fetch --tags;brew update;brew upgrade;brew upgrade --cask --greedy;brew cleanup;brew cu -afy --cleanup;brew cleanup'
...
alias genvenv3='virtualenv -p python3.8'
...
alias j10='sdk use java $(find_java_sdk 10)'
alias j11='sdk use java $(find_java_sdk 11)'
alias j12='sdk use java $(find_java_sdk 12)'
alias j13='sdk use java $(find_java_sdk 13)'
alias j14='sdk use java $(find_java_sdk 14)'
alias j16='sdk use java $(find_java_sdk 16)'
alias j8='sdk use java $(find_java_sdk 8)'
alias j9='sdk use java $(find_java_sdk 9)'
...
alias k8proxy='cd ${WDIR}/private/k8s && make run_proxy'
alias k='kubectl'
alias kall='k get all --all-namespaces'
alias kc='kubectx'
alias kconf='kubeconf'
...
alias megamount='mkdir -p "${HOME}/mnt/mega" && touch "${HOME}/mnt/mega.out" && nohup rclone cmount --daemon-timeout 100000m --vfs-cache-mode full Mega:/ "${HOME}/mnt/mega" >> "${HOME}/mnt/mega.out" 2>&1 &'
alias mk='minikube'
...
alias tml='tmux ls'
alias tref='print_and_copy "${TESLA_REFERRAL}"'
alias trp='cd ${WDIR}/tools'
alias tz='date "+%z %Z"'
alias unow='print_and_copy $(date +%s)'
alias update='macsoftwareupdate; bup; yarnupdate; npm install -g npm; sdkmanupdate'
...
alias urr='[ ! -z ${RECOMMENDED_READINGS_GIST_ID} ] && (gist -u ${RECOMMENDED_READINGS_GIST_ID} ${RECOMMENDED_READINGS}) || echo "Run envify"'
...
alias vi='vim'
alias vimpluginupdate='(cd "${HOME}/.vim_runtime" && python update_plugins.py)'
...
alias ycminstall='(cd "${HOME}/.vim_runtime/sources_non_forked/YouCompleteMe" && git submodule update --init --recursive && ./install.py --clang-completer --gocode-completer --tern-completer --omnisharp-completer)'
