...
export PATH="${CUSTOM_BIN_DIR}:${HOME}/.yarn/bin:${HOME}/.config/yarn/global/node_modules/.bin:/opt/homebrew/bin:${HOME}/.gem/ruby/2.6.0/bin:/usr/local/opt/openssl/bin:/usr/local/opt/curl/bin:${PATH}"
...
alias ag1ref='print_and_copy "${ATHLETIC_GREENS_REFERRAL}"'
...
alias rm_yarn_home='cd ~ && rm -rf node_modules .yarn yarn.lock'
...
alias superocd='ocd && update && gupallin "${HOME}" && zgen update <<< "n" &> /dev/null && gck && rm_yarn_home'
alias t='tig'
...
alias ycminstall='(cd "${HOME}/.vim_runtime/my_plugins/YouCompleteMe" && git submodule update --init --recursive && ./install.py --clang-completer --gocode-completer --tern-completer --omnisharp-completer)'
