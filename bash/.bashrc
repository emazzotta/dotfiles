...
alias ai_llama='av && envify && interpreter -y'
...
alias audiofix='envify && av && audiotags_manager.py --full-tag-optimizer'
alias audiofixall='envify && av && find . -type f \( -name "*.mp3" -o -name "*.wav" -o -name "*.flac" \) -exec audiotags_manager.py --full-tag-optimizer {} \;'
...
alias pwautocompress='envify && pwcompress -c 0 "$PASSWORD_ZIPS"'
...
alias superocd='ocd && update && rmraycastclipboard && rm_act_cache && rmmac && gupallin "$HOME" && zgen update <<< "n" &> /dev/null && gck "$HOME" && create_backups'
alias t='lazygit'
alias tags_dj_fix='envify && av && audiotags_manager.py --autofix'
...
alias tsort='dj && cd Manu/Todo_Sort_Tracks'
alias tz='date "+%z %Z"'
